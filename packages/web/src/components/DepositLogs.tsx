...
import { getDepositLogs } from "../app/actions";

export function DepositLogs() {
  const t = useTranslations("DepositLogs");
  ...
  const fetchLogs = () => {
    ...
    setLoading(true);
    getDepositLogs(signer.address, page, limit)
      .then((result) => {
        console.log("Fetched deposit logs:", result);
        ...
        <TableBody>
          {logs.map((log) => (
            <TableRow key={`${log.transactionHash}-${log.timestamp}`}>
              <TableCell>{formatAddress(log.favoriteAddress)}</TableCell>
